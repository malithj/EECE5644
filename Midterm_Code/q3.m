...
high = 1;
N = 10;
v_sigma = 0.1;       % choose sigma in a way which will increase noise (standard distribution sigma: 1)
...
for gamma = logspace(start, stop, stop - start + 1)
    for run = 1:totalRuns
        ...
        syms a b c d;
        eqd = a * (sum(x.^3)) + b * (sum(x.^2)) + c * (sum(x)) + d * ((v_sigma^2 / (gamma^2)) + N) == sum(y);
        eqc = a * (sum(x.^4)) + b * (sum(x.^3)) + d * (sum(x)) + c * ((v_sigma^2 / (gamma^2)) + (sum(x.^2))) == sum(x.*y);
        eqb = a * (sum(x.^5)) + c * (sum(x.^3)) + d * (sum(x.^2)) + b * ((v_sigma^2 / (gamma^2)) + (sum(x.^4))) == sum((x.^2).*y);
        eqa = b * (sum(x.^5)) + c * (sum(x.^4)) + d * (sum(x.^3)) + a * ((v_sigma^2 / (gamma^2)) + (sum(x.^6))) == sum((x.^3).*y);
